<?php
if(!class_exists("DOROCreativeAgencyWordPressThemeBase")) {
	class DOROCreativeAgencyWordPressThemeBase {
    	public $key = "8C86BD6B7BA49472";
    	private $product_id = "10";
    	private $product_base = "doro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_doro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=doro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
